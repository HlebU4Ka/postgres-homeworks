...
with psycopg2.connect(database="postgres_homeworks",
                      user="postgres",
    ...
    try:
        with conn.cursor() as cur:
            with open(os.path.join('..', 'homework-1', 'north_data', 'customers_data.csv'), 'r',
                ...
                for i in csv_text:
                    cur.execute("INSERT INTO customers_data VALUES (%s, %s, %s)", tuple(i))
            ...
            with open(os.path.join('..', 'homework-1', 'north_data', 'employees_data.csv'), 'r', encoding='utf-8') \
                ...
                # запись данных в таблицу
                for i in csv_text:
                    cur.execute('INSERT INTO employees_data VALUES (%s, %s, %s, %s, %s, %s)', tuple(i))
            ...
            with open(os.path.join('..', 'homework-1', 'north_data', 'orders_data.csv'), 'r', encoding='utf-8') \
                ...
                # запись данных в таблицу
                for i in csv_text:
                    cur.execute('INSERT INTO orders_data VALUES (%s, %s, %s, %s, %s)', tuple(i))
